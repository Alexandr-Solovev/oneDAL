...
sycl::event train_service_kernels<Float, Bin, Index, Task>::initialize_tree_order(
    ...
    // tree_order is allocated for the whole tree block, which can be larger than tree_count
...
sycl::event train_service_kernels<Float, Bin, Index, Task>::do_level_partition_by_groups(
    ...
    ONEDAL_ASSERT(node_list.get_count() == node_count * impl_const_t::node_prop_count_);
    ONEDAL_ASSERT(tree_order.get_count() == ctx.tree_in_block_ * ctx.tree_order_stride_);
    ONEDAL_ASSERT(tree_order_buf.get_count() == ctx.tree_in_block_ * ctx.tree_order_stride_);
...
sycl::event train_service_kernels<Float, Bin, Index, Task>::get_oob_row_list(
    ...
    // row_list is allocated for the whole tree block, which can be larger than node_count
