...
public:
    ...
    static constexpr auto engine_type = engine_v<EngineType>;
    ...
    auto get_engine(std::int64_t seed) {
        auto rng_engine = oneapi_engine<engine_type>(this->get_queue(), seed);
        return rng_engine;
    }

    auto allocate_arrays(std::int64_t elem_count) {
        auto& q = this->get_queue();
        auto val_gpu = ndarray<DataType, 1>::empty({ elem_count });
        auto val_host = ndarray<DataType, 1>::empty(q, { elem_count }, sycl::usm::alloc::device);
    ...
    auto allocate_arrays_device(std::int64_t elem_count) {
        auto& q = this->get_queue();
        auto val_gpu_1 = ndarray<DataType, 1>::empty(q, { elem_count }, sycl::usm::alloc::device);
        auto val_gpu_2 = ndarray<DataType, 1>::empty(q, { elem_count }, sycl::usm::alloc::device);

        return std::make_tuple(val_gpu_1, val_gpu_2);
    }

    auto allocate_arrays_host(std::int64_t elem_count) {
        auto val_host_1 = ndarray<DataType, 1>::empty({ elem_count });
        auto val_host_2 = ndarray<DataType, 1>::empty({ elem_count });

        return std::make_tuple(val_host_1, val_host_2);
    }

    void check_results_host(const ndarray<DataType, 1>& val_host_1,
                            const ndarray<DataType, 1>& val_host_2) {
        const DataType* val_host_1_ptr = val_host_1.get_data();

        const DataType* val_host_2_ptr = val_host_2.get_data();
    ...
    void check_results_device(const ndarray<DataType, 1>& val_gpu_1,
                              const ndarray<DataType, 1>& val_gpu_2) {
        const auto val_gpu_host_1 = val_gpu_1.to_host(this->get_queue());
        const DataType* val_gpu_host_1_ptr = val_gpu_host_1.get_data();

        const auto val_gpu_host_2 = val_gpu_2.to_host(this->get_queue());
        const DataType* val_gpu_host_2_ptr = val_gpu_host_2.get_data();
    ...
    void check_results(const ndarray<DataType, 1>& val_gpu, const ndarray<DataType, 1>& val_host) {
        const DataType* val_host_ptr = val_host.get_data();

        const auto val_gpu_host = val_gpu.to_host(this->get_queue());
        const DataType* val_gpu_host_ptr = val_gpu_host.get_data();
...
TEMPLATE_LIST_TEST_M(rng_test, "rng cpu vs gpu", "[rng]", rng_types) {
    ...
    auto [arr_host, arr_gpu] = this->allocate_arrays(elem_count);
