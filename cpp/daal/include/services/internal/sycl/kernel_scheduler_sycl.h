...
public:
    ...
    {
        return sycl::make_kernel<cl::sycl::backend::opencl>(_clKernelRef.get(), ctx);
    }
...
{
public:
    SyclKernelSchedulerArgHandler(cl::sycl::queue & queue, sycl::handler & handler, UsmPointerStorage & storage, size_t argumentIndex,
...
{
    return sycl::nd_range<1>(cl::sycl::range<1>(r.global().upper1()), sycl::range<1>(r.local().upper1()));
}
...
private:
    ...
    void passArguments(cl::sycl::queue & queue, sycl::handler & cgh, UsmPointerStorage & storage, const KernelArguments & args,
                       Status & status) const
    {
